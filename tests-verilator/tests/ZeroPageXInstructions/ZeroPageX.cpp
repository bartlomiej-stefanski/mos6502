...
class ZeroPageXInstructions : public Mos6502
{
public:
  ZeroPageXInstructions() : Mos6502(Mos6502::JumpVector{.reset = program_start})
  {
    // Write something on first two pages.
...
TEST_F(ZeroPageXInstructions, StoreRegisterTests)
{
  ...
  tick(3);
  {
    ...
    EXPECT_EQ(bus->get< u8 > (sta_addr + *prev_state.x), cpu->REG_A);
  }
}
...
TEST_F(ZeroPageXInstructions, ShiftOpTest)
{
  ...
  tick(3);
  {
    ...
    expect_bus_write(256 - (rol_addr - *prev_state.x), rol(rol_addr - *prev_state.x, *prev_flags.carry));
  }
  ...
  tick(3);
  {
    ...
    expect_bus_write(256 - (ror_addr - *prev_state.x), ror(ror_addr - *prev_state.x, *prev_flags.carry));
  }
}
