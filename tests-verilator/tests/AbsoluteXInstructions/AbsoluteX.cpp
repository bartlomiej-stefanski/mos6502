...
class AbsoluteXInstructions : public Mos6502
{
public:
  inline static constexpr Addr MemoryPage{0xa000};
  inline static constexpr Addr RamAddr{0x2000};

  AbsoluteXInstructions() : Mos6502(Mos6502::JumpVector{.reset = program_start})
  {
...
TEST_F(AbsoluteXInstructions, ShiftOpTest)
{
  ...
  tick(4);
  {
    ...
    expect_bus_write(rol_addr + *prev_state.x, rol((rol_addr + *prev_state.x) & 0xFF, *prev_flags.carry));
  }
  ...
  tick(4);
  {
    ...
    expect_bus_write(ror_addr + *prev_state.x, ror((ror_addr + *prev_state.x) & 0xFF, *prev_flags.carry));
  }
}
